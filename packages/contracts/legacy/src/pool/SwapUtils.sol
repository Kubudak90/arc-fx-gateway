 ...
 * Vendored from Saddle Finance (MIT). SafeMath removed — 0.8 has built-in overflow checks.
 * OZ imports updated to v5 paths. Upgradeable deps removed.
...
library SwapUtils {
    ...
    function getYD(
        ...
        c =
            (c * d * AmplificationUtils.A_PRECISION) /
            (nA * numTokens);
    ...
    function getD(
        ...
        for (uint256 i = 0; i < MAX_LOOP_LIMIT; i++) {
            ...
            prevD = d;
            d =
                (((nA * s) / AmplificationUtils.A_PRECISION + dP * numTokens) *
                    d) /
    ...
    function addLiquidity(
        ...
        if (v.totalSupply != 0) {
            ...
            for (uint256 i = 0; i < pooledTokens.length; i++) {
                ...
                fees[i] =
                    (feePerToken *
                        MathUtils.difference(idealBalance, newBalances[i])) /
                    FEE_DENOMINATOR;
                self.balances[i] =
                    newBalances[i] -
    ...
    function removeLiquidityImbalance(
        ...
        uint256[] memory fees = new uint256[](pooledTokens.length);
        {
            ...
            for (uint256 i = 0; i < pooledTokens.length; i++) {
                uint256 idealBalance = (v.d1 * v.balances[i]) / v.d0;
                uint256 difference = MathUtils.difference(
                    idealBalance,
                    balances1[i]
                );
